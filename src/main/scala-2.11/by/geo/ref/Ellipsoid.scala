...
case class Ellipsoid(a: Double, f: Double, GM: Double) {
  ...
  val b = a * (1 - f)
  private val e = FastMath.sqrt(a2 - FastMath.pow(b, 2))
  val e1 = e / a
  val e2 = e / b
  ...
  val q = FastMath.pow(a, 3) * w2 / GM
  private val ee = FastMath.pow(e2, 2)

  val gammaE = GM * (1.0 - 3 * q / 2 + f + FastMath.pow(f, 2) - 3.0 * f * q / 7.0
    + FastMath.pow(f, 3) - 125.0 * q * FastMath.pow(f, 2)
    / 294.0) / a2

  val gammaP = GM / a2 - 2.0 * w2 * b * (1.0 - 2.0 * ee * (e2 - FastMath.atan(e2))
    / ((3.0 + ee) * FastMath.atan(e2) - 3.0 * e2)) / 3.0

  private val q0 = 0.5 * ((1.0 + 3.0 / FastMath.pow(e2, 2)) * FastMath.atan(e2) - 3.0 / e2)

  val k = (b * gammaP - a * gammaE) / (a * gammaE)

  private val C = (1.0 / 3.0) * (1.0 - (2.0 / 15.0) * (m * e2 / q0))

  val rMean = (2.0 * a + b) / 3.0
  val gammaMean = (2.0 * gammaE + gammaP) / 3.0
